from storygenapp.storygen.dbobjects.DirectAssertion import DirectAssertion
from storygenapp.storygen.dbobjects.AssertionType import AssertionType
from storygenapp.storygen.dbobjects.LikedPagesAndEvents import LikedPagesAndEvents
...
class AssertionModel:
    ...
    def getLikedPagesAndEvents(self):
        ...
        try:
            with conn.cursor() as cursor:
                sql = "SELECT AST.assertion_type, A.parameters FROM assertions A LEFT JOIN assertions_types AST ON A.assertion_type = AST.id WHERE AST.assertion_type LIKE 'likes' OR AST.assertion_type LIKE 'event'"
                cursor.execute(sql)
                row = cursor.fetchone()
                likeseventslist = []
    ...
    def getAssertionType(self, assertiontype):
        ...
        try:
            with conn.cursor() as cursor:
                sql = "SELECT * FROM assertions_types WHERE assertion_type = '" + assertiontype + "'"
                cursor.execute(sql)
                row = cursor.fetchone()
    ...
    def getAssertionTypeID(self, assertion_type):
        ...
        try:
            with conn.cursor() as cursor:
                sql = "SELECT id FROM assertions_types WHERE assertion_type = '" + assertion_type + "'"
                cursor.execute(sql)
                row = cursor.fetchone()
                id = row['id']
        finally:
    ...
    def insertAssertionTypes(self):
        ...
        with open('C:\\Users\\denis_000\\PycharmProjects\\storygen\\assertions_types.csv') as csvfile:
            ...
            for row in reader:
                ...
                sql = "INSERT INTO `assertions_types`(`id`, `user_pref`, `assertion_type`, `parameter`) VALUES(%s, %s, %s, %s)"
                cursor = conn.cursor()
                cursor.executemany(sql, [(row['id'], row['user_pref'], row['assertion_type'], row['parameter'])])
